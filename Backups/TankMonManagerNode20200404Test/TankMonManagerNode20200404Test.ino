/*
  ...
  - modify initTanks to remove hard coded tank numbers & blynk pin refs
...
void initTanks()
{
  for (int t = 0; t <= NUMTANKS - 1; t++)      // set max volume and alarm level on all tanks
  {
    ...
    tanks[t].loAlarm = tanks[t].depth * 0.10;
    tanks[t].hiAlarm = tanks[t].depth * 1.10;
  }
...
void setup()
{
  ...
  Serial.begin(115200);
  int msgn = sprintf(nodename, "MNTM-ESP8266-%X", ESP.getChipId());
  connectWiFi();
  setupNTP();
  printTimestamp();
  setupOTA();
