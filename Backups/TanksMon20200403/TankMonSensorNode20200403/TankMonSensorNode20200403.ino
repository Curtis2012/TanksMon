...
void pingTanks()
{
  ...
  for (int t = 0; t <= NUMTANKS - 1; t++)
  {
    ...
    tanks[t].liquidDepthAvg += tanks[t].liquidDepth;
...
void sendTankData()
{
  ...
  for (int t = 0; t <= NUMTANKS - 1; t++)
  {
    tanks[t].liquidDepthAvg /= tanks[t].pingCount;
...
void setup()
{
  ...
  nodename = "SNTM-ESP8266-" + String(ESP.getChipId(), HEX);
