...
/*
  ...
  2020-04-18 Moved to Visual Studio for IDE, incorporated into Tanks Mon solution
...
void initTanks()
{
  ...
  for (i = 0; i < NUMTANKS; i++)
  {
    ...
    if (tanks[i].tankType = 'P') tanks[i].timeOut = 600000; // set timeOut to 10 min since propane tank sensor defaults to 5 min sleep
  }
}
...
void setup()
{
  ...
  msgn = sprintf(nodeName, "MNTM-ESP8266-%X", ESP.getChipId());
  ...
  setupMQTT(MDNS.IP(hostEntry), MDNS.port(hostEntry), true, handleMQTTmsg);
...
void loop() {
  Blynk.run();
  if (!mqttClient.connected()) connectMQTT(true, MDNS.IP(hostEntry));
  mqttClient.loop();
  tickTimers();
}
